...
// Reached from the link in a Supabase "recovery" email (sent by
// resetPasswordForEmail on Login.tsx). Same mechanics as AcceptInvite:
// supabase-js's detectSessionInUrl already establishes a session from the
// URL before this component mounts, so this just checks it exists and lets
...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // against the now-updated session, matching the pattern used in
      // AcceptInvite for GitHub Pages' base path.
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#fafafa] p-6">
        <div className="w-full max-w-[480px] bg-white rounded-[20px] border-2 border-[#f0f2fb] p-8 lg:p-10 text-center">
          ...
          <p className="font-['Montserrat',sans-serif] text-[16px] text-[#666] mb-6">
            Peça um novo link de redefinição de senha na tela de login.
          </p>
          <button
